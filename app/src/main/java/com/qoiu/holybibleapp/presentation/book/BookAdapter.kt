...
class BookAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder<BookUi> =
        when (viewType) {
            0 -> BooksViewHolder.Testament(R.layout.testament.makeView(parent),collapseListener)
            1-> BooksViewHolder.Base(R.layout.book_layout.makeView(parent),bookListener)
            2 -> BaseViewHolder.Fail(R.layout.fail_fullscreen.makeView(parent), retry)
    ...
    abstract class BooksViewHolder(view: View) : BaseViewHolder<BookUi>(view) {
        ...
        class Base(view: View, private val bookListener: BookListener) : BooksViewHolder.Info(view) {
