...
class BibleAdapter(
    ...
    private val collapseListener: BibleViewHolder.CollapseListener
) :
    ...
    fun update(new: List<BookUi>) {
        val result = DiffUtil.calculateDiff(DiffUtilCallback(books,new))
        books.clear()
        books.addAll(new)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BibleViewHolder =
        when (viewType) {
            0 -> BibleViewHolder.Testament(R.layout.testament.makeView(parent),collapseListener)
                ...
                book.showCollapsed(object : BookUi.CollapseMapper{
                    override fun show(collapsed: Boolean) {
                        val iconId = if(collapsed){
                            R.drawable.ic_expand_less_black_24
                        }else{
                            R.drawable.ic_expand_more_black_24
                        }
